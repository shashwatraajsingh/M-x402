...
export default function AIIDEIntegrationPage() {
  const [content, setContent] = useState<string>("");
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch('/docs/guides/ai-ide-integration.md')
      .then(res => res.text())
      .then(text => {
        setContent(text);
        setLoading(false);
      ...
      <div className="container mx-auto px-6 py-12 max-w-4xl">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-zinc-200 p-8 md:p-12">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-4xl font-bold text-zinc-900">AI IDE Integration</h1>
              <p className="text-lg text-zinc-600 mt-2">Use Cursor IDE and AI assistants to integrate Monad x402</p>
            </div>
          </div>
